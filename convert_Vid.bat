...
ptime ffmpeg -i "%%~A" -vf "scale=-2:720" -c:v hevc_nvenc -pix_fmt yuv420p -crf 23 -c:a aac "E:\Photos\tmp_videos\%%~nxB_%%~nA.mkv"
@REM ptime ffmpeg  -i "%%~A" -s hd720 -c:v hevc_nvenc -pix_fmt yuv420p -crf 23 -c:a aac "E:\Photos\tmp_videos\%%~nxB_%%~nA.mkv"
REM ptime ffmpeg  -i "%%~A" -s hd720 -c:v libx265 -pix_fmt yuv420p -crf 23 -c:a aac "E:\Photos\tmp_videos\%%~nxB_%%~nA.mkv"
