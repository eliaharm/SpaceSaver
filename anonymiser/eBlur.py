...
cascadeFront = cv2.CascadeClassifier("anonymiser\\lbpcascade_frontalface_improved.xml")
cascadeProfile = cv2.CascadeClassifier("anonymiser\\lbpcascade_profileface.xml")


def face_blur(src_img, dest_img, zoom_in=1):
    """
    Recognize and blur all faces in the source image file,
    then save as destination image file.
    """
    ...
    if hasattr(face_locationsFront, "size"):
        print(
            "%s:There are %s FRONT faces at " % (src_img, len(face_locationsFront)),
            face_locationsFront,
        )
    if hasattr(face_locationsProfile, "size"):
        print(
            "%s:There are %s PROFILE faces at " % (src_img, len(face_locationsProfile)),
            face_locationsProfile,
        )
    if not hasattr(face_locationsFront, "size"):
        face_locations = face_locationsProfile
    elif not hasattr(face_locationsProfile, "size"):
        face_locations = face_locationsFront
    else:
        face_locations = np.concatenate(
            (face_locationsFront, face_locationsProfile), axis=0
        )

    if hasattr(face_locations, "size"):
        print(
            "%s:There are %s faces at " % (src_img, len(face_locations)), face_locations
        )
    else:
...
def face_blur_df(src_img, dest_img, zoom_in=1):
    """
    Recognize and blur all faces in the source image file,
    then save as destination image file.
    """

    embedding = DeepFace.represent(img_path=src_img)
    sys.stdout.write("%s:processing... \r" % (embedding))
    ...
    for root, subdirs, files in os.walk(src_dir):
        ...
        for filename in files:
            ...
            if ext == ".jpg":
                ...
                face_blur_df(srcfile_path, destfile_path)


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("faceblur v1.0.0 (c) telesoho.com")
        print("Usage:python eBlur <src-image/directory> <dest-image/directory>")
    else:
        if os.path.isfile(sys.argv[1]):
            # face_blur(sys.argv[1], sys.argv[2])
            face_blur_df(sys.argv[1], sys.argv[2])
        else:
            blur_all_photo(sys.argv[1], sys.argv[2])

# blur_all_photo(
#     "C:/Users/elia/Desktop/to_share/test",
#     "C:/Users/elia/Desktop/to_share_X",
# )
